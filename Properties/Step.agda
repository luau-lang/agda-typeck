...
open import Luau.Syntax using (Block; Expr; nil; var; val; addr; bool; function_is_end; block_is_end; _$_; local_←_; return; done; _∙_; name; fun; arg; num; binexp; +; -; *; /; <; >; <=; >=; ==; ~=; ··; str)
...
open import Luau.RuntimeType using (valueType; function; num)
...
binOpStep (num m) + nil = error₂ (+ (λ ()))
binOpStep (num m) + (addr a) = error₂ (+ (λ ()))
binOpStep (num m) + (num n) = step (num (primFloatPlus m n)) (+ m n)
binOpStep (num m) + (bool b) = error₂ (+ (λ ()))
binOpStep (num m) + (str x) = error₂ (+ (λ ()))
binOpStep (num m) - (str x) = error₂ (- (λ ()))
binOpStep (num m) * (str x) = error₂ (* (λ ()))
binOpStep (num m) / (str x) = error₂ (/ (λ ()))
binOpStep (num m) < (str x) = error₂ (< (λ ()))
binOpStep (num m) > (str x) = error₂ (> (λ ()))
binOpStep (num m) == (str x) = step (bool false) (== (num m) (str x))
binOpStep (num m) ~= (str x) = step (bool true) (~= (num m) (str x))
binOpStep (num m) <= (str x) = error₂ (<= (λ ()))
binOpStep (num m) >= (str x) = error₂ (>= (λ ()))
binOpStep (bool b) + w = error₁ (+ (λ ()))
binOpStep nil - w = error₁ (- (λ ()))
binOpStep (addr a) - w = error₁ (- (λ ()))
binOpStep (num x) - nil = error₂ (- (λ ()))
binOpStep (num x) - (addr a) = error₂ (- (λ ()))
binOpStep (num x) - (num n) = step (num (primFloatMinus x n)) (- x n)
binOpStep (num x) - (bool b) = error₂ (- (λ ()))
binOpStep (bool b) - w = error₁ (- (λ ()))
binOpStep nil * w = error₁ (* (λ ()))
binOpStep (addr a) * w = error₁ (* (λ ()))
binOpStep (num m) * nil = error₂ (* (λ ()))
binOpStep (num m) * (addr a) = error₂ (* (λ ()))
binOpStep (num m) * (num n) = step (num (primFloatDiv m n)) (* m n)
binOpStep (num m) * (bool b) = error₂ (* (λ ()))
binOpStep (bool b) * w = error₁ (* (λ ()))
binOpStep nil / w = error₁ (/ (λ ()))
binOpStep (addr a) / w = error₁ (/ (λ ()))
binOpStep (num m) / nil = error₂ (/ (λ ()))
binOpStep (num m) / (addr a) = error₂ (/ (λ ()))
binOpStep (num m) / (num n) = step (num (primFloatTimes m n)) (/ m n)
binOpStep (num m) / (bool b) = error₂ (/ (λ ()))
binOpStep (bool b) / w = error₁ (/ (λ ()))
binOpStep nil < w = error₁ (< (λ ()))
binOpStep (addr a) < w = error₁ (< (λ ()))
binOpStep (num m) < nil = error₂ (< (λ ()))
binOpStep (num m) < (addr a) = error₂ (< (λ ()))
binOpStep (num m) < (num n) = step (bool (primFloatLess m n)) (< m n)
binOpStep (num m) < (bool b) = error₂ (< (λ ()))
binOpStep (bool b) < w = error₁ (< (λ ()))
binOpStep nil > w = error₁ (> (λ ()))
binOpStep (addr a) > w = error₁ (> (λ ()))
binOpStep (num m) > nil = error₂ (> (λ ()))
binOpStep (num m) > (addr a) = error₂ (> (λ ()))
binOpStep (num m) > (num n) = step (bool (primFloatLess n m)) (> m n)
binOpStep (num m) > (bool b) = error₂ (> (λ ()))
binOpStep (bool b) > w = error₁ (> (λ ()))
...
binOpStep (num m) <= nil = error₂ (<= (λ ()))
binOpStep (num m) <= (addr a) = error₂ (<= (λ ()))
binOpStep (num m) <= (num n) = step (bool (primFloatLess m n or primFloatEquality m n)) (<= m n)
binOpStep (num m) <= (bool b) = error₂ (<= (λ ()))
binOpStep (bool b) <= w = error₁ (<= (λ ()))
binOpStep nil >= w = error₁ (>= (λ ()))
binOpStep (addr a) >= w = error₁ (>= (λ ()))
binOpStep (num m) >= nil = error₂ (>= (λ ()))
binOpStep (num m) >= (addr a) = error₂ (>= (λ ()))
binOpStep (num m) >= (num n) = step (bool (primFloatLess n m or primFloatEquality m n)) (>= m n)
binOpStep (num m) >= (bool b) = error₂ (>= (λ ()))
binOpStep (bool b) >= w = error₁ (>= (λ ()))
binOpStep (str x) + w = error₁ (+ (λ ()))
binOpStep (str x) - w = error₁ (- (λ ()))
binOpStep (str x) * w = error₁ (* (λ ()))
binOpStep (str x) / w = error₁ (/ (λ ()))
binOpStep (str x) < w = error₁ (< (λ ()))
...
binOpStep (num x) ·· y = error₁ (BinOpError.·· (λ ()))
binOpStep (bool x) ·· y = error₁ (BinOpError.·· (λ ()))
binOpStep (str x) ·· nil = error₂ (·· (λ ()))
binOpStep (str x) ·· (addr y) = error₂ (·· (λ ()))
binOpStep (str x) ·· (num y) = error₂ (·· (λ ()))
binOpStep (str x) ·· (bool y) = error₂ (·· (λ ()))
binOpStep (str x) ·· (str y) = step (str (primStringAppend x y)) (·· x y)
...
stepᴱ H (_ $ _) | value (num m) refl | value w refl = error (FunctionMismatch (num m) w (λ ()))
stepᴱ H (_ $ _) | value (bool b) refl | value w refl = error (FunctionMismatch (bool b) w (λ ()))
stepᴱ H (_ $ _) | value (str x) refl | value w refl = error (FunctionMismatch (str x) w (λ ()))
