...
data ¬Language where
  ...
  scalar-function-ok : ∀ {S t u} → (Scalar S) → ¬Language S (t ↦ u)
  scalar-function-err : ∀ {S t} → (Scalar S) → ¬Language S (function-err t)
  scalar-function-tgt : ∀ {S t} → (Scalar S) → ¬Language S (⟨⟩↦ t)
